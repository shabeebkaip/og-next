...
const YourGoal = () => {
    ...
    const [findProgram, setfindProgram] = useState([])
    useEffect(() => {
      globalGetService('findProgram')
          .then(response => {
            setfindProgram(response.data)
          })
  }, [])
  const array = [
    findProgram?.image,
            ...
            <div className='flex flex-col items-start justify-center text-start 2xl:p-8 lg:p-7'>
                ...
                <div className='2xl:max-w-[550px] lg:max-w-[450px] 2xl:mt-9 mt-6 md:mb-0 mb-16'>
                    {findProgram.description?.map((item, index) => (
                        <p key={index} className='font-normal 2xl:text-[26px] 2xl:leading-[30.5px] w-full lg:text-[19px] text-[26px] leading-[30.5px] lg:leading-tight'>Lorem ipsum, or lipsum as it is sometimes known, is dummy text used in laying out print, graphic or web designs.</p>
                    ))}
