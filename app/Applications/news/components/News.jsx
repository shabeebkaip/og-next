...
import { setNewsList } from "@/app/redux/newsReducer";
import { useDispatch, useSelector } from "react-redux";
import { getNewsListApi } from "../api";
import { setNewsPagination } from "@/app/redux/newsReducer";

const News = () => {
    const { newsList, newsPagination } = useSelector(state => state.news)
    ...
    const loadmore = () => {
        getNewsListApi({ page: newsPagination?.page + 1 }).then(res => {
            dispatch(setNewsList([...newsList, ...res.data]))
    ...
    const handleCategory = (id) => {
        ...
        if (id === null) {
            getNewsListApi().then(res => dispatch(setNewsList(res)))
        } else {
            getNewsListApi({ category: id }).then(res => dispatch(setNewsList(res.data)))
        }
    }
    return (
        <div className="container mx-auto pb-8">
            <div className="flex flex-col ">
                <div className="relative flex flex-row justify-center gap-4 px-10 md:justify-end">
                    <p className={`text-base font-medium ${selectedCategoryId === null && 'bg-[#FF8500]  text-white px-3 rounded-xl'}`} onClick={() => handleCategory()}>
                        all
                    </p>
                    {
